...
export function clearSessionCookie(res) {
  // path + maxAge:0 alcanza en la mayoria de los navegadores, pero se
  // agrega tambien "expires" en el pasado como refuerzo — algunos clientes
  // (proxies, navegadores viejos) respetan expires de forma mas confiable
  // que maxAge para invalidar una cookie de inmediato.
...
export async function getSession() {
  ...
  // Log temporal y seguro para verificar en los logs de Vercel que cada
  // sesion resuelve al usuario correcto — nunca imprime el token.
  console.log(
    `[session] userId=${user.id} email=${user.email} github_login=${user.github_login || "(no vinculado)"}`
  );

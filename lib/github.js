...
export async function deployAtomicCommit({
  ...
  // tiene ni un commit. Ambos casos significan lo mismo para nosotros: no
  // hay padre, este va a ser el primer commit.
  const ref = await gh(
  ...
  return { commitSha: commit.sha, url: `https://github.com/${owner}/${repo}/commit/${commit.sha}` };
}

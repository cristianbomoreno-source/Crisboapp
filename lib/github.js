...
// Los objetos de Git Data (blobs) que se acaban de crear pueden tardar un
// instante en estar disponibles para llamadas subsiguientes que los
// referencian (por ejemplo armar un arbol que apunta a un blob subido
// milisegundos antes) — GitHub responde con un 404 "Not Found" transitorio
// en ese caso puntual, que no tiene que ver con permisos ni con que el
// objeto realmente no exista. Se usa solo en los 3 pasos de cierre de
// finalizeCommitFromBlobs (crear arbol, crear commit, mover la rama), que
// son justamente los que referencian blobs recien creados.
async function ghWriteRetryingPropagation(path, token, options) {
  const delaysMs = [500, 1000, 2000];
  for (let attempt = 0; ; attempt++) {
    try {
      return await ghWrite(path, token, options);
    } catch (err) {
      if (err.status === 404 && attempt < delaysMs.length) {
...
export function getAuthorizeUrl(redirectUri, state) {
  const params = new URLSearchParams({
    ...
    // "workflow" es necesario ademas de "repo": sin el, GitHub rechaza
    // (con un 404 generico en /git/trees, no un 403 claro) cualquier commit
    // que toque archivos bajo .github/workflows/ — como los proyectos que
    // incluyen un GitHub Actions workflow (ej. para disparar un deploy hook
...
// Crea/actualiza UN archivo via la API de Contents de GitHub (no la de Git
// Data). Es el unico camino permitido en un repo que todavia no tiene
// ningun commit — por eso lo usa el bootstrap. Usa ghWrite para heredar la
// misma pausa + reintentos ante el limite secundario.
export async function ghWriteContents({ owner, repo, token, path, base64Content, branch, message }) {
  return ghWrite(`/repos/${owner}/${repo}/contents/${path}`, token, {
    method: "PUT",
...
// Verifica que un conjunto de blobs (identificados por su sha) todavia
// existan como objetos en el repo. Se usa SOLO para los blobs "resumidos"
// de un progreso guardado en el navegador de un intento anterior — esos son
// los unicos con riesgo real de haber sido borrados por el garbage
// collector de GitHub, porque se subieron sueltos (sin llegar a integrarse
// a ningun commit) y puede haber pasado bastante tiempo antes de
// reintentar. Los blobs "sin cambios" (ya parte de un commit existente) y
// los recien creados en el mismo intento no necesitan esta verificacion.
export async function findMissingBlobs({ owner, repo, token, entries }) {
  const missing = [];
  for (const entry of entries) {
    const exists = await gh(`/repos/${owner}/${repo}/git/blobs/${entry.sha}`, token)
      .then(() => true)
      .catch((e) => {
...
export async function getExistingTreeMap({ owner, repo, branch, token }) {
...
export async function finalizeCommitFromBlobs({ owner, repo, branch, token, blobs, message }) {
  ...
  // Log de diagnostico: si /git/trees falla mas adelante, esto deja en los
  // logs de Vercel la lista exacta de paths+shas que se mando, para poder
  // identificar a simple vista cualquier path raro (caracteres especiales,
  // path vacio, duplicados, etc.) sin tener que adivinar.
  console.log(
    `[finalizeCommitFromBlobs] ${owner}/${repo}@${branch} — armando arbol con ${blobs.length} entradas: ` +
      JSON.stringify(blobs.map((b) => ({ path: b.path, sha: b.sha })))
  );

  let tree;
  try {
    tree = await ghWriteRetryingPropagation(`/repos/${owner}/${repo}/git/trees`, token, {
      method: "POST",
      body: JSON.stringify({
    ...
    err.treeEntries = blobs.map((b) => ({ path: b.path, sha: b.sha }));
    throw err;
  }

  const commit = await ghWriteRetryingPropagation(`/repos/${owner}/${repo}/git/commits`, token, {
    method: "POST",
    body: JSON.stringify({ message, tree: tree.sha, parents: parentSha ? [parentSha] : [] }),
  });

  if (ref) {
    await ghWriteRetryingPropagation(`/repos/${owner}/${repo}/git/refs/heads/${encodeURIComponent(branch)}`, token, {
      method: "PATCH",
      body: JSON.stringify({ sha: commit.sha, force: false }),
    });
  } else {
    await ghWriteRetryingPropagation(`/repos/${owner}/${repo}/git/refs`, token, {
      method: "POST",
...
export async function deployAtomicCommit({
  ...
  // Caso borde: un repositorio recien creado, sin ningun commit todavia,
  // NO acepta crear blobs sueltos via la API de Git Data — ni uno solo,
  // aunque sea para "sembrarlo". El unico camino permitido en un repo
  // vacio es la API de Contents (subir un archivo con su contenido
  // directo). Por eso esto se resuelve ANTES del bucle normal de blobs,
  // usando el contenido real del primer archivo (nunca un sha de blob que
  // nunca pudo llegar a crearse).
  const ref = await getBranchRef({ owner, repo, branch, token });
  if (!ref && files.length > 0) {
    onProgress?.({ stage: "bootstrap" });
    const first = files[0];
    const bootstrapPath = first.path.split("/").map(encodeURIComponent).join("/");
    const created = await ghWrite(`/repos/${owner}/${repo}/contents/${bootstrapPath}`, token, {
      method: "PUT",
      body: JSON.stringify({
        message: "Inicializa el repositorio",
        content: first.buffer.toString("base64"),
        branch,
      }),
    });
    blobs.push({ path: first.path, sha: created.content.sha });
    added++;
    startIndex = 1;
    onProgress?.({ stage: "blobs", index: 1, total: files.length, path: first.path, sha: created.content.sha });
  }

  onProgress?.({ stage: "blobs", index: startIndex, total: files.length });
